...
- The picker also closes on a click outside it, on <kbd>Escape</kbd>, or when focus moves to an element outside it
...
See the [Vanilla Calendar Pro documentation](https://vanilla-calendar.pro/docs/reference/settings) for all available options. Some options need an [extension](#extensions).

`vcpOptions.type` sets the calendar view (for example `'week'`, which needs the `weeks` extension). `displayMonthsCount` takes priority: set it above `1` and the datepicker forces the `'multiple'` type, overriding `vcpOptions.type`.

### Extensions

Vanilla Calendar Pro ships its optional features as extensions. Bootstrap registers only the `months` extension, which `displayMonthsCount` needs. This keeps the other features out of your bundle.

Register an extension before you use an option that needs it. Otherwise, Vanilla Calendar Pro throws an error that names the missing extension.

<BsTable>
| Extension | Adds | Options that need it |
| --- | --- | --- |
| `weeks` | Week view, collapse to the selected week, and week numbers | `type: 'week'`, `enableCollapse`, `enableWeekNumbers`, `onClickWeekDay` |
| `motion` | Animated paging that respects `prefers-reduced-motion`, and swipe navigation with a mouse, touch, or pen | `animation`, `enableSwipe` |
| `time` | Time selection | `selectionTimeMode` |
| `annotations` | Date popups and range tooltips | `popups`, `onCreateDateRangeTooltip` |
</BsTable>

Use `Datepicker.registerExtensions()` to add extensions to every datepicker, including those that the data API creates. Call it before the page finishes loading, so that it runs before the inline datepickers initialize:
...
After you register them, the options work through data attributes too:

```html
<div data-bs-toggle="datepicker" data-bs-inline="true" data-bs-vcp-options='{"animation": true, "enableWeekNumbers": true}'></div>
```

To add extensions to one datepicker only, pass them in `vcpOptions.extensions`. Bootstrap merges them with the registered extensions.

```js
import { Datepicker } from 'bootstrap'
import { time } from 'vanilla-calendar-pro'
...
Import the extensions from the same copy of `vanilla-calendar-pro` that Bootstrap uses. `bootstrap.bundle.js` includes its own copy with only the `months` extension. To use other extensions, use `bootstrap.js` with a bundler or an import map.
</Callout>
...
| `registerExtensions(...extensions)` | Static method to register [Vanilla Calendar Pro extensions](#extensions) for all new datepicker instances |
</BsTable>
