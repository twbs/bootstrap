...
- **Rebuilt the carousel on CSS scroll snap.** The slide engine no longer uses `float` + `translateX` class juggling or a custom swipe handler—`.carousel-inner` is now a native horizontal scroll-snap container, so sliding, touch dragging, momentum, and keyboard scrolling come from the browser. The markup is unchanged (`.carousel` &rarr; `.carousel-inner` &rarr; `.carousel-item`), and the public JavaScript API (`next`, `prev`, `to`, `cycle`, `pause`) plus the `slide.bs.carousel` / `slid.bs.carousel` events are preserved.
  - **New capabilities:** show multiple slides at once, reveal a “peek” of adjacent slides, gaps, center mode, and variable-width slides—all via CSS custom properties (`--carousel-items`, `--carousel-gap`, `--carousel-peek`), the `.carousel-center` / `.carousel-auto` variants, and responsive `.carousel-items-*` utilities.
  ...
  - **`.carousel-fade`** is now a stacked-opacity mode that upgrades to a [View Transition](https://developer.mozilla.org/en-US/docs/Web/API/View_Transition_API) where supported.
  - **`touch: false`** now applies `touch-action: pan-y` instead of skipping a swipe handler; the `touch` option no longer relies on the (removed) internal swipe utility.
