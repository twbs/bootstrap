...
$alert-tokens: map.merge(
  ( ... built-in defaults ... ),
  $alert-tokens
);
```

The `() !default` declaration receives your `with()` overrides. Then `map.merge()` layers your entries on top of the built-in defaults, so unspecified keys keep their original values.
