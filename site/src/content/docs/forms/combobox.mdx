...
The combobox uses CSS custom properties prefixed with `--combobox-*` for the toggle, and inherits `--menu-*` properties from the Menu component for the menu overlay. See the [source SCSS](https://github.com/twbs/bootstrap/blob/main/scss/forms/_combobox.scss) for the full list of available tokens.
