$(function () {
  ...
  QUnit.test('should check for closest matching toggle', function (assert) {
    ...
    var $inputBtn2 = $btn2.find('input').eq(0)
    ...
    $inputBtn2.trigger('click')

    assert.ok(!$btn1.hasClass('active'), 'first click: btn1 does not have active class')
    assert.ok(!$btn1.find('input').prop('checked'), 'first click: btn1 is not checked')
    assert.ok($btn2.hasClass('active'), 'first click: btn2 has active class')
    assert.ok($inputBtn2.prop('checked'), 'first click: btn2 is checked')

    $inputBtn2.trigger('click') // clicking an already checked radio should not un-check it

    assert.ok(!$btn1.hasClass('active'), 'second click: btn1 does not have active class')
    assert.ok(!$btn1.find('input').prop('checked'), 'second click: btn1 is not checked')
    assert.ok($btn2.hasClass('active'), 'second click: btn2 has active class')
    assert.ok($inputBtn2.prop('checked'), 'second click: btn2 is checked')
  })
