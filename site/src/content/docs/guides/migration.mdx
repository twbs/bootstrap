...
- **Refactor checks, radios, and switches.**
  ...
  - Checkboxes now have a wrapping element (`.check`) and draw the checked and indeterminate marks with a CSS `mask-image` on a `::after` pseudo-element tinted with `background-color: currentcolor`—no inline SVG in the DOM. Radios and switches do not have a wrapping element.
