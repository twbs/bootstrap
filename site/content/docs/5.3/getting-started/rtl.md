...
Do you want to make this process automatic and covering several edge cases that involve having both directions in a single stylesheet? Then consider using [PostCSS RTLCSS](https://github.com/elchininet/postcss-rtlcss) as a [PostCSS](https://github.com/postcss/postcss) plugin to process your source files. PostCSS RTLCSS uses [RTLCSS](https://rtlcss.com) behind the scenes to manage the direction flipping process, but it separates the flipped declarations in rules with a different prefix for LTR and RTL, something that allows you to have both directions in the same stylesheet file. Doing this, it is possible to change from LTR to RTL and vice versa just changing [the dir attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/dir) of the page (or even changing a specific class if you configure the plugin to do that).

{{< callout warning >}}
**Important things to take into account** when using PostCSS RTLCSS to build a combined LTR and RTL implementation:

1. It is recommended that you add [the dir attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/dir) to the `html` element, in this way the whole page will be affected when you change the direction, and also make sure you add [the lang attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/lang) accordingly.
2. Having a single bundle with both directions will increase the size of the final stylesheet (in average a 20%-30%): consider some [optimization]({{< docsref "/customize/optimize" >}}).
3. Take into account that PostCSS RTLCSS is not compatible with `/* rtl:remove */` directives because it doesn't remove any CSS rule. You should replace your `/* rtl:remove */`, `/* rtl:begin:remove */` and `/* rtl:end:remove */` directives by `/* rtl:ignore */`, `/* rtl:begin:ignore */` and `/* rtl:end:ignore */` directives respectively. These directives will ignore the rule and will not create an RTL counterpart (same result as the `remove` ones in `RTLCSS`).
{{< /callout >}}
