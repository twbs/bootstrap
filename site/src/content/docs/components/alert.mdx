...
| Method | Description |
| --- | --- |
| `close` | Closes an alert by removing it from the DOM. If the `.fade` and `.show` classes are present on the element, the alert will fade out before it is removed. **Returns a promise that resolves once the alert is removed** (i.e. after the `closed.bs.alert` event fires). |
