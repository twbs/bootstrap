...
Use the [margin]([[docsref:/utilities/margin]]) and [padding]([[docsref:/utilities/padding]]) utilities to control how elements and components are spaced and sized. Bootstrap includes a 13-step scale for spacing utilities, based on a default `$spacer` value of `1rem`. Choose values for all viewports (e.g., `.me-5` for `margin-inline-end: 1rem`), or use responsive variants to target specific viewports (e.g., `.md:me-5` starting at the `md` breakpoint).
