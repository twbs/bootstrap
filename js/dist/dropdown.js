(function (global, factory) {
  ...
  (global.Dropdown = factory(global.Data,global.EventHandler,global.Manipulator,global.Popper,global.SelectorEngine,global.Util));
  ...
  function () {
    ...
    Dropdown._dataApiKeydownHandler = function _dataApiKeydownHandler(event) {
      ...
      if (/input|textarea/i.test(event.target.tagName) ? event.which === SPACE_KEYCODE || event.which !== ESCAPE_KEYCODE && (event.which !== ARROW_DOWN_KEYCODE && event.which !== ARROW_UP_KEYCODE || SelectorEngine.closest(event.target, Selector.MENU) !== null) : !REGEXP_KEYDOWN.test(event.which)) {
        return;
      }
      ...
      if (!isActive && (event.which !== ESCAPE_KEYCODE || event.which !== SPACE_KEYCODE) || isActive && (event.which === ESCAPE_KEYCODE || event.which === SPACE_KEYCODE)) {
        if (event.which === ESCAPE_KEYCODE) {
          var toggle = SelectorEngine.findOne(Selector.DATA_TOGGLE, parent);
