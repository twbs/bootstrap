...
- **Removed the combined `--*-transition` tokens.** Buttons, nav links, accordions, form controls, range thumbs, and floating labels each dropped their composed `--*-transition` value for a `--*-transition-property` / `--*-transition-duration` / `--*-transition-timing` trio. The old token packed a whole property list and one duration into the `transition` shorthand, where the duration binds to the last property only—so a button’s `color`, `background-color`, and `border-color` never actually animated. Components now emit the `transition-*` longhands, which repeat one duration and easing across every listed property. Replace overrides of `--bs-btn-transition` and friends with the matching `-duration` and `-timing` tokens.
