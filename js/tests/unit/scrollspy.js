$(function () {
  ...
  QUnit.test('should allow to use passed as option offset method', function (assert) {
    assert.expect(4)
    ...
    var testOffsetMethod = function (type) {
      ...
      var navbarHtml =
        ...
        + '</nav>'
      var contentHtml =
          '<div class="content"' + (type === 'data' ? ' data-spy="scroll" data-target="#navbar-offset-method-menu" data-offset="0" data-position="false"' : '') + ' style="position: relative; overflow: auto; height: 100px">'
      ...
      if (type === 'js') $content.bootstrapScrollspy({ offset: 0, target: '.navbar', position: false })
      else if (type === 'data') $(window).trigger('load.bs.scrollspy.data-api')

      var $target = $('#div-' + type + 'm-2')
      var scrollspy = $content.data('bs.scrollspy')

      assert.ok(scrollspy.offsets[1] === $target.offset().top, 'offsed method with ' + type + ' option')
      assert.ok(scrollspy.offsets[1] !== $target.position().top, 'position method with ' + type + ' option')
