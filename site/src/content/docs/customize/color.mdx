  ...
  {getData('colors').map((color) => {
   ...
   <div class="d-contents grid-full">
    ...
    {additionalColors.map((color) => {
      return (
        <div key={`${color}`} class="mb-3 font-monospace small">
...
<div class="grid gap-0 small" style={{gridTemplateColumns: 'repeat(11, 1fr)', minWidth: '0'}}>
  {getData('colors').map((color) => {
     return (
       <div key={color.name} style={{ gridColumn: '1 / -1', display: 'contents' }}>
         <h5 class="d-block text-capitalize" style={{ gridColumn: '1 / -1' }}>{color.name.replace('-', ' ')}</h5>

         {["050", ...getSequence(100, 900, 100), 950].map((value) => {
           return (
             <div key={`${color.name}-${value}`} class="mb-3 font-monospace">
               <div class="py-4 mb-1" style={{backgroundColor: `var(--new-${color.name}-${value})`}}></div>
               <div>{value}</div>
             </div>
           )
         })}
