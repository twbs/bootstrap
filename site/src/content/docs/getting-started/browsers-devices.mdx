...
Bootstrap’s CSS is written against the modern platform baseline: [`light-dark()`](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/light-dark) for [color modes]([[docsref:/customize/color-modes]]), `oklch()` and `color-mix()` for the [color system]([[docsref:/customize/color]]), native CSS nesting, container queries, and `:has()`. Each floor sits on the first version of that engine with native `light-dark()`. Below it, our build pipeline lowers `light-dark()` to a custom-property polyfill, and that polyfill breaks `data-bs-theme` dark mode. This is why we cannot lower the floors further. Raising them would drop users and gain us nothing.

Together these targets cover about 87% of global browser usage. Run `npx browserslist --coverage` in a clone to check the current figure yourself.
...
| Windows, macOS, Linux | Chrome 123+, Edge 123+, Firefox 120+; Safari 17.5+ on macOS |
</BsTable>
