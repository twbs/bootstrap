...
An alert fades out before it is removed. The plugin adds `.hiding` for the length of the transition, so no extra classes are needed in your markup. Add `.alert-instant` to remove the alert at once instead. See [Transitions]([[docsref:/customize/transitions]]) for the tokens that tune the fade.
...
| Method | Description |
| --- | --- |
| `close` | Closes an alert by removing it from the DOM. The alert fades out first, unless it has `.alert-instant`. **Returns a promise that resolves once the alert is removed** (i.e. after the `closed.bs.alert` event fires). |
