...
The collapse JavaScript plugin is used to show and hide content. Buttons or anchors are used as triggers that are mapped to specific elements you toggle. The plugin only adds and removes `.show`—the CSS animates the size from `0` to the content size. Given how CSS handles animations, you cannot use `padding` on a `.collapse` element. Instead, use the class as an independent wrapping element.

The animation needs [`interpolate-size`](https://developer.mozilla.org/en-US/docs/Web/CSS/interpolate-size). Browsers without it open and close the element at once. See [Transitions]([[docsref:/customize/transitions]]) to change the timing or to remove it.
...
The collapse plugin supports horizontal collapsing. Add the `.collapse-horizontal` modifier class to transition the inline size instead of the block size, and set a `width` on the immediate child element. Feel free to write your own custom Sass, use inline styles, or use our [width utilities]([[docsref:/utilities/width]]).
...
The collapse plugin utilizes two classes to handle the heavy lifting:

- `.collapse` hides the content
- `.collapse.show` shows the content

These classes can be found in `_transitions.scss`. The plugin toggles `.show` and then waits for the CSS transition to finish before it fires `shown.bs.collapse` or `hidden.bs.collapse`.
...
| `hide` | Hides a collapsible element. **Returns a promise that resolves once the collapsible element is hidden** (i.e. after the `hidden.bs.collapse` event fires). |
| `show` | Shows a collapsible element. **Returns a promise that resolves once the collapsible element is shown** (i.e. after the `shown.bs.collapse` event fires). |
| `toggle` | Toggles a collapsible element to shown or hidden. **Returns a promise that resolves once the collapsible element is shown or hidden** (i.e. after the `shown.bs.collapse` or `hidden.bs.collapse` event fires). |
</BsTable>
