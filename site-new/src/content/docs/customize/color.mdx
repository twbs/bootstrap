    ...
    if ((color.name != "white") && (color.name != "gray") && (color.name != "gray-dark")) {
      return (
        <div class="col-md-4 mb-3">
          ...
          {/* TODO: create an helper to handle this array from 100 to 900 */}

          {[100, 200, 300, 400, 500, 600, 700, 800, 900].map((intRange) => {
            return (
              <div class={`p-3 bd-${color.name}-${intRange}`}>${color.name}-{intRange}</div>
            )
          })}
        </div>
      )
