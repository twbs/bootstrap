...
Nest multiple `.form-field`s inside an outer `.form-field` to group related controls under a shared label. The outer wrapper stays in single-column mode because it has no direct `.check`, `.radio`, or `.switch` child, while each inner `.form-field` gets its own two-column layout.
...
Add `.form-field-group` to each inner `.form-field` for a card-style treatment that highlights the selected option. Wrap the inner fields in a container with tighter spacing for a more compact look.
...
Add `data-bs-validate` to your `<form>` to opt in to `:user-invalid` and `:user-valid` styling. These pseudo-classes activate after the user interacts with a control. Add `novalidate` to suppress native browser validation tooltips while keeping the constraint validation API available.
