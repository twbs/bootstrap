...
Add `data-bs-theme="dark"` to the `.carousel` for darker controls and indicators. Controls are inverted compared to their default white fill with the `filter` CSS property. Controls have additional Sass variables that customize the `color` and `background-color`.
